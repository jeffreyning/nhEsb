...
$(function(){
	$('#infoList').datagrid({
		...
		url:"<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=nhesb_sys_list&groovySubName=getInfoList4Page",
		columns:[[
					{
						field : 'id',
						title : 'id',
					...
					},
					{
						...
						formatter:function(val,row){
							return "<a href='<%=path%>/nh-micro-jsp/nhesb-page/SysStatusInfo.jsp?sysId="+row.meta_key+"'>查看服务状态</a>";
						}
					}					
...
function addOne(){
	...
	var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=nhesb_sys_list&groovySubName=createInfo";
...
function updateOne(){
	var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=nhesb_sys_list&groovySubName=updateInfo";
...
function remove(){
	...
	if(sels == ''|| sels==null){
		...
		if(result == true){
			var querydata = $('#searchForm').serializeObject();
			var id = sels.id;
			var url="<%=path%>/NhEsbServiceServlet?cmdName=Groovy&subName=nhesb_sys_list&groovySubName=delInfo";
			url=url+"&id="+id;
		...
		<form id="updateOneForm" novalidate method="post" action="">
			...
			<table id="updateUserTable"
				...
				<tr>
					...
					<td><select name="remark" id="remark" type="text" value="" /></td>
				</tr>
